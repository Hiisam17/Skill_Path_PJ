...
export class ProgressQueueService {
  ...
  private cacheQueue = new Map<
    string,
    { userId: string; roadmapSkillId: number }
  >();
  ...
  async flushQueueToDatabase() {
    ...
    this.logger.log(
      `Bắt đầu đồng bộ ${batch.size} bản ghi tiến độ xuống DB...`,
    );

    const records = Array.from(batch.values());

    try {
      const uniqueRoadmapSkillIds = [
        ...new Set(records.map((r) => r.roadmapSkillId)),
      ];
      ...
      for (const userId of uniqueUserIds) {
        for (const roadmapId of affectedRoadmapIds) {
          syncPromises.push(
            this.progressService.syncRoadmapProgressPercentage(
              userId,
              roadmapId,
            ),
          );
      ...
      this.logger.log(
        `Đồng bộ thành công! Đã cập nhật % cho ${syncPromises.length} cặp User-Roadmap.`,
      );
    } catch (error) {
